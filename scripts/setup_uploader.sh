...
# Rerun setup_uploader.sh any time to upgrade -- it pulls the latest code,
# updates dependencies, and rewrites this wrapper in place.
set -euo pipefail
args=("\$@")
has_config=false
for a in "\$@"; do
...
# PYTHONPATH (not "cd \$INSTALL_DIR") so "uploader.upload_job" resolves as a
# module regardless of which directory you run "chamber" from, without
# disturbing relative paths in your own arguments (e.g. a relative file to
# upload, or --out) -- those still resolve against your actual cwd.
exec env PYTHONPATH="$INSTALL_DIR" "$INSTALL_DIR/.venv/bin/python" -m uploader.upload_job "\${args[@]}"
EOF
